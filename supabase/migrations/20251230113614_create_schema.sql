/*
  # Create PerformIQ Database Schema

  1. New Tables
    - `orgs` - Organizations with name and timestamps
    - `app_users` - User accounts synced from Supabase Auth
    - `org_members` - Organization membership with roles
    - `integrations` - Integration settings per org/provider
    - `integration_accounts` - OAuth tokens and account details
    - `external_identities` - Mapping external users to app users
    - `raw_events` - Original event payloads from providers
    - `events` - Normalized events in unified format
    - `sync_state` - Sync progress and cursor tracking
    - `audit_log` - Audit trail for actions

  2. Indexes
    - Performance indexes on frequently queried columns
    - Composite indexes for org + provider queries
  ...
  4. Security
    ...
    - CHECK constraints on enums (provider, status, type)
