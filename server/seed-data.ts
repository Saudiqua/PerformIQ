...
export async function seedData() {
  ...
  // Create employees with varying performance patterns
  ...
  // Generate alerts for struggling employees and recent drops
  const allMetrics = [];
  ...
  const alerts = await storage.getAlertsByManager(managerIds[0]);
  console.log(`Created ${alerts.length} alerts for first manager`);
}
